...
const BASELINE: &str = "verification/1.2-performance-baseline.json";
...
fn plan_handover(root: &Path, args: &HandoverArgs) -> Result<PlannedHandover> {
    ...
    verify_canonical_operational_receipt(&canonical_root, &operational_validation.1)?;
    ...
    let payload = HandoverPlanPayload {
        ...
        limitations: vec![
            ...
            "Live-provider and performance evidence retain their recorded current, stale, deferred or NOT RUN status.".into(),
        ],
    };
...
fn verify_canonical_operational_receipt(root: &Path, expected: &[u8]) -> Result<()> {
    let output = Command::new("uv")
        ...
        .stdin(Stdio::null())
        .output()
        .context("MINCO-HANDOVER-003: run canonical operational-evidence validator")?;
    if !output.status.success() || output.stdout != expected {
        bail!(
            "MINCO-HANDOVER-003: operational validation receipt differs from canonical validator output"
        );
    }
    Ok(())
}
...
fn validate_provider_ledger(
    ...
    for profile in profiles {
        ...
        if state == "current" {
            let source = profile.get("source_revision").and_then(toml::Value::as_str);
            let contact = profile
                .get("provider_contact")
                .and_then(toml::Value::as_bool);
            let cleanup = profile.get("cleanup_state").and_then(toml::Value::as_str);
            let retained = profile
                .get("retained_resources")
                .and_then(toml::Value::as_array);
            if source != Some("receipt_bound")
                || contact != Some(true)
                || cleanup != Some("verified_absent")
                || retained.is_none_or(|items| !items.is_empty())
            {
                bail!(
                    "MINCO-HANDOVER-003: current provider evidence lacks exact source or cleanup proof"
                );
            }
        }
...
mod tests {
    ...
    #[test]
    fn forged_operational_receipt_differs_from_canonical_validator_output() {
        let root = Path::new(env!("CARGO_MANIFEST_DIR"))
            .join("../..")
            .canonicalize()
            .unwrap();
        let bytes = fs::read(root.join(OPERATIONAL_VALIDATION)).unwrap();
        assert!(verify_canonical_operational_receipt(&root, &bytes).is_ok());
        let forged = bytes
            .windows(b"\"current_provider_profiles\": 0".len())
            .position(|window| window == b"\"current_provider_profiles\": 0")
            .map(|index| {
                let mut forged = bytes.clone();
                let value_index = index + b"\"current_provider_profiles\": ".len();
                forged[value_index] = b'9';
                forged
            })
            .unwrap();
        assert!(verify_canonical_operational_receipt(&root, &forged).is_err());
    }
