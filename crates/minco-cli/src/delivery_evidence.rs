...
fn collect_source_files(
    ...
    for entry in entries {
        ...
        if metadata.is_dir() {
            collect_source_files(root, &path, extra_exclusions, files)?;
...
fn current_source_entries(root: &Path, extra_exclusions: &[PathBuf]) -> Result<Vec<SourceEntry>> {
    ...
    collect_source_files(&root, &root, &extra, &mut files)?;
...
pub fn verify_current_source_manifest(root: &Path) -> Result<VerifiedSourceManifest> {
    ...
    let manifest_path = root.join("verification/source-manifest.json");
    let bytes = fs::read(&manifest_path).context("read current source manifest")?;
    ...
    Ok(VerifiedSourceManifest {
        ...
        file: FileDigest {
            path: "verification/source-manifest.json".into(),
            sha256: sha256(&bytes),
            bytes: u64::try_from(bytes.len()).context("source manifest size overflow")?,
        },
    })
}
...
mod secure {
    use super::{
        ExactInput, MAX_OUTPUT_BYTES, OutputSpec, PublicationResult, Result, bail, sha256,
        validate_relative,
    };
    use anyhow::Context;
    use rustix::{
...
mod tests {
    ...
    fn source_exclusions_match_the_canonical_policy() {
        assert_eq!(source_exclusions().len(), 15);
