...
const FEEDBACK_ATTACHMENT_ROOT: &str = "target/minco/feedback-attachments";
    ...
    match args.command {
        ...
        FeedbackCommand::Attachment {
            ...
            let canonical_root = root
                .canonicalize()
                .with_context(|| format!("resolve project root {}", root.display()))?;
            let output_path = resolve_attachment_output_file(&canonical_root, &output)?;
            let relative = PathBuf::from(relative_utf8(&canonical_root, &output_path)?);
            let result = publish_create_only(
                &canonical_root,
                vec![OutputSpec {
...
fn resolve_attachment_output_file(root: &Path, requested: &Path) -> Result<PathBuf> {
    resolve_output_file(root, requested, Path::new(FEEDBACK_ATTACHMENT_ROOT)).with_context(|| {
        format!("raw feedback attachments must remain under {FEEDBACK_ATTACHMENT_ROOT}")
    })
}
...
mod tests {
    ...
    fn task_identifiers_and_output_paths_fail_closed() {
        ...
        assert!(
            resolve_attachment_output_file(&root, Path::new("crates/minco-cli/build.rs")).is_err()
        );
        assert!(
            resolve_attachment_output_file(
                &root,
                Path::new("target/minco/feedback-attachments/thread/attachment.bin"),
            )
            .is_ok()
        );
    }
