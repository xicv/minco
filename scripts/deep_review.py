...
def source_files(pattern: str, root: Path = ROOT) -> list[Path]:
    return [path for path in sorted(root.rglob(pattern)) if not is_excluded(path, root)]
...
def main() -> int:
    ...
    for path in source_files("*.rs"):
        ...
        is_integration_test = "tests" in path.relative_to(ROOT).parts
        production = not is_integration_test and (
            "#[cfg(test)]" not in text or text.index("#[cfg(test)]") > 0
        )
        pre_test = text.split("#[cfg(test)]", 1)[0] if production else ""
    ...
    report = {
        ...
        "compiler_verification": {
            ...
            "evidence": "See VERIFICATION.md for the separately executed compiler and runtime gates.",
        },
    }
    rendered = json.dumps(report, indent=2) + "\n"
    for output in [
        ROOT / "target/minco/deep-review.json",
        ROOT / "verification/deep-review.json",
    ]:
