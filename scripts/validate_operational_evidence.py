...
class Validator:
    ...
    def validate_measurement(self, value: Any, label: str, minimum: dict[str, Any], path: Path) -> None:
        ...
        if not isinstance(requests, int) or isinstance(requests, bool) or requests < minimum.get("api_requests", 1):
        ...
        elif not finite_number(error_rate) or abs(error_rate - failures / requests) > 1e-12:
    ...
    def validate_provider(self, document: dict[str, Any], path: Path) -> None:
        ...
        for profile in profiles:
            ...
            if not isinstance(profile.get("max_age_days"), int) or profile["max_age_days"] <= 0:
            ...
            age_current = self.effective_date is not None and (self.effective_date - observed.date()).days <= profile.get("max_age_days", 0)
    ...
    def validate_capabilities(self, document: dict[str, Any], path: Path) -> None:
        ...
        for candidate in candidates:
            ...
            if any(not source.startswith("https://") for source in candidate.get("upstream_sources", [])):
