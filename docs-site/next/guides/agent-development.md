...
Minco 1.5.0 packages nine focused application workflows for Codex and Claude
Code. Every skill now teaches the 1.5 assurance release boundary: use typed
side-effect fakes for application behavior, keep topology-cost and measured
quality evidence explicit, and never promote deterministic projection checks
into model or human-review evidence. The `minco-waffo-payments` skill retains
