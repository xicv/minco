...
identity, sessions, object storage, events, notifications, audit, Feedback, and
static sites. The catalog also includes PostgreSQL/SQLite adapters and native
Lambda HTTP/SQS runtimes.
...
- [All 16 built-in components](../plugins/)
...
`cargo minco inspect --json`, `explain`, task commands, deterministic plans,
stable diagnostics, and evidence receipts make the application legible to both
people and coding agents without giving an agent hidden production authority.

See [Testing and evidence](../reference/testing).
