...
AUDIT_DYNAMODB_TABLE_NAME=<distinct audit table reference>
...
When the durable audit capability is selected, the plan derives a separate
encrypted, on-demand audit table with point-in-time recovery and retained
deletion/replacement policy. Orders and their canonical audit action commit in
one cross-table transaction. See [Durable action auditing](./auditing) for
privacy, relationship fanout, query and lifecycle boundaries.
