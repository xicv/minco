...
Code. Every skill is version-matched to this manual, retains cumulative release
feature coverage and includes the maintenance release boundary. The
`minco-waffo-payments` skill continues to teach the provider-specific checkout,
token, webhook and evidence boundary. Skills select Minco commands and source
authorities; they do not add a hosted agent runtime or replace the application
graph.
