---
title: Protect traffic at the gateway
description: Apply explicit API Gateway request-rate and burst targets before Lambda invocation.
---

# Protect traffic at the gateway
...
against the reviewed deployment plan rather than asking applications to repeat
method/path strings.
...
`ThrottlingBurstLimit` as an int32 provider field. Unknown operation IDs, plans
that repeat an operation ID, policies whose overrides resolve to one route key,
and rendered templates whose stage markers are ambiguous all fail closed
instead of silently choosing one setting.
...
- adds no extra AWS resource beyond settings on stages Minco already renders; and
