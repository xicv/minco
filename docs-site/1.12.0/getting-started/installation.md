...
published baseline is 1.12.0. The release and current development require Rust
1.97.1. Use exact published 1.12.0 packages for applications and the repository-pinned
...
The last command should print `minco 1.12.0`. Contributors reviewing unreleased
...
cargo add minco@1.12.0 --features sqlx-sqlite,test
...
- Apple Container 1.2.x is preferred for fresh Minco-owned PostgreSQL and
  Rustack services on qualified Apple silicon hosts. Docker remains the
  fallback and supports Compose-only custom services; neither is needed for an
