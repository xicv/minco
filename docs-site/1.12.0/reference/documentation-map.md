...
| Enforce request assertions | [Enforce request contracts](../guides/contract-request-validation) | generated validation, ValidatedJson, permissions, scopes, 400, 422 |
| Browser and native clients | [Browser and native clients](../guides/mobile-api) | mobile API, PKCE, OAuth, CORS, retry, offline, deprecation, app integrity |
| Exact HTTP resource shapes | [Resource API reference](./resource-api) | envelope, pagination, filter, sort, precondition |
| Exact request boundary | [HTTP request boundary](./http-request-boundary) | supported assertions, limits, taxonomy, request ID, timeout, body limit |
...
| Events, rich mail, and delivery intent | [Events and notifications](../guides/events-and-notifications) | outbox, email, SES, Mailpit, webhook, notification |
| Durable action history | [Durable action auditing](../guides/auditing) | audit, action log, actor, changes, journal, relay, archive, retention |
...
| Project-scoped support workflow | [Ticketing support entry](../guides/ticketing) | ticketing, portal, launcher, handoff, requester, agent, internal note |
...
| Protect ingress traffic | [Protect traffic at the gateway](../guides/traffic-policy) | throttling, rate, burst, route settings, 429, best effort |
| Compress API responses | [Compress HTTP delivery](../guides/http-compression) | gzip, Accept-Encoding, Vary, 1 KiB threshold, BREACH, Lambda binary |
...
- [HTTP request boundary](./http-request-boundary)
...
| request validator / form request | `generated validation`, `ValidatedJson`, `validation_failed` |
