...
| `GET` | `/orders/{orderId}/audit` | `listOrderAuditHistory` | Separate permission, bounded opaque cursor, survives soft delete |
...
Each successful create, update and delete also produces one semantic audit
intent. SQL profiles accept it in the source transaction before a relay writes
the distinct ledger. The DynamoDB profile commits the source item and separate
audit-table projections in one conditional transaction. See
