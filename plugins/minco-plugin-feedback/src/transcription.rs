...
#[async_trait]
pub trait FeedbackTranscriber: Send + Sync + std::fmt::Debug {
    async fn transcribe(
        &self,
...
#[cfg(feature = "openai-transcription")]
impl std::fmt::Debug for OpenAiTranscriber {
    fn fmt(&self, formatter: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        formatter
...
impl FeedbackTranscriber for OpenAiTranscriber {
    async fn transcribe(
        ...
        if request.bytes.is_empty() {
            return Err(TranscriptionError::InvalidAudio("audio is empty".into()));
        }
...
impl CommandTranscriber {
    /// Creates a direct-process transcription adapter.
    ///
    /// Arguments containing `{input}` receive the temporary audio path. When no
    /// argument contains the placeholder, the path is appended as the final
    /// argument. The process is launched directly without a shell.
    #[must_use]
...
impl FeedbackTranscriber for CommandTranscriber {
    async fn transcribe(
        ...
        let input = directory.path().join(format!("feedback-audio.{extension}"));
...
mod tests {
    ...
    #[tokio::test]
    async fn disabled_transcriber_fails_explicitly() {
        let result = DisabledTranscriber
    ...
    #[cfg(feature = "command-transcription")]
    #[test]
    fn command_arguments_substitute_or_append_the_input_path() {
        use std::path::Path;

        let input = Path::new("/tmp/example.webm");
        let substituted = CommandTranscriber::new("whisper")
            .with_arguments(["--file", concat!("{", "input", "}")])
            .rendered_arguments(input);
        assert_eq!(
            substituted[1],
            std::ffi::OsString::from("/tmp/example.webm")
        );

        let appended = CommandTranscriber::new("whisper")
            .with_arguments(["--model", "tiny"])
            .rendered_arguments(input);
        assert_eq!(appended.last(), Some(&input.as_os_str().to_os_string()));
    }

    #[cfg(feature = "command-transcription")]
    #[test]
    fn command_audio_extension_is_bounded_and_content_type_aware() {
        assert_eq!(safe_audio_extension("voice.m4a", "audio/mp4"), "m4a");
        assert_eq!(safe_audio_extension("voice", "audio/ogg"), "ogg");
        assert_eq!(
            safe_audio_extension("voice.bad-extension!", "audio/webm"),
            "webm"
        );
    }
}
