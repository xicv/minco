...
`minco-plugin-payments-waffo` is an opt-in Minco integration for Waffo Pancake hosted checkout, signed server actions, read-only GraphQL queries, and standard HTTP webhooks. It has been published since Minco `1.3.0`; the `1.8.0` candidate preserves the same provider-specific API and authority boundary. Object-transfer changes do not change or prove payment-provider behavior.
