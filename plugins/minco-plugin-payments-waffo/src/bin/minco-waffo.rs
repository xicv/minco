...
struct CliSecretResolver {
    ssm: OnceCell<aws_sdk_ssm::Client>,
}

impl fmt::Debug for CliSecretResolver {
    fn fmt(&self, formatter: &mut fmt::Formatter<'_>) -> fmt::Result {
        formatter
            .debug_struct("CliSecretResolver")
            .field("ssm_initialized", &self.ssm.get().is_some())
            .finish()
    }
}

impl CliSecretResolver {
    async fn ssm_client(&self) -> Result<&aws_sdk_ssm::Client, WaffoError> {
        self.ssm
            .get_or_try_init(|| async {
                let sdk = aws_config::defaults(BehaviorVersion::latest()).load().await;
                Ok::<_, WaffoError>(aws_sdk_ssm::Client::new(&sdk))
            })
            .await
    }
}
...
impl SecretResolver for CliSecretResolver {
    async fn resolve(&self, reference: &SecretReference) -> Result<SecretValue, WaffoError> {
        match reference {
            ...
            SecretReference::SystemsManagerParameter { name } => {
                let output = self
                    .ssm_client()
                    .await?
                    .get_parameter()
                ...
                output
                    ...
                    .map(SecretValue::new)
                    .ok_or(WaffoError::SecretResolution)
            }
        }
    }
}
...
async fn run(cli: Cli) -> Result<()> {
    ...
    let resolver = CliSecretResolver::default();
    ...
    match cli.command {
        ...
        Command::Doctor => {
            let _client = loaded.service.client(&resolver).await?;
            let webhook_key = if configuration.webhook_public_key_provider().is_some() {
                let _verifier = loaded.service.webhook_verifier(&resolver).await?;
                "valid"
            } else {
                "not_configured"
            };
        ...
        Command::WebhookVerify { signature, body } => {
            ...
            let verifier = loaded.service.webhook_verifier(&resolver).await?;
