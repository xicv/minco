...
use crate::{WaffoError, identifier::validate_short_id};
...
pub struct RawWaffoConfiguration {
    ...
    #[serde(default = "default_webhook_past_tolerance_seconds")]
    webhook_past_tolerance_seconds: u64,
    #[serde(default = "default_webhook_future_tolerance_seconds")]
    webhook_future_tolerance_seconds: u64,
...
impl Default for RawWaffoConfiguration {
    fn default() -> Self {
        Self {
            ...
            webhook_past_tolerance_seconds: default_webhook_past_tolerance_seconds(),
            webhook_future_tolerance_seconds: default_webhook_future_tolerance_seconds(),
...
pub struct WaffoConfiguration {
    ...
    webhook_past_tolerance: Duration,
    webhook_future_tolerance: Duration,
...
impl fmt::Debug for WaffoConfiguration {
    fn fmt(&self, formatter: &mut fmt::Formatter<'_>) -> fmt::Result {
        formatter
            ...
            .field("webhook_past_tolerance", &self.webhook_past_tolerance)
            .field("webhook_future_tolerance", &self.webhook_future_tolerance)
...
impl TryFrom<RawWaffoConfiguration> for WaffoConfiguration {
    ...
    fn try_from(raw: RawWaffoConfiguration) -> Result<Self, Self::Error> {
        ...
        if !(1..=3_600).contains(&raw.webhook_past_tolerance_seconds) {
            return Err(WaffoError::InvalidConfiguration(
                "webhook_past_tolerance_seconds must be between 1 and 3600",
            ));
        }
        if !(1..=300).contains(&raw.webhook_future_tolerance_seconds) {
            return Err(WaffoError::InvalidConfiguration(
                "webhook_future_tolerance_seconds must be between 1 and 300",
            ));
        }
        ...
        if webhook_public_key.is_some() && raw.store_id.is_none() {
            return Err(WaffoError::MissingWebhookVerificationConfiguration);
        }
        let registration_requested = webhook_url.is_some() || !raw.webhook_events.is_empty();
        if registration_requested
            && (webhook_url.is_none() || raw.webhook_events.is_empty() || raw.store_id.is_none())
        {
        ...
        Ok(Self {
            ...
            webhook_past_tolerance: Duration::from_secs(raw.webhook_past_tolerance_seconds),
            webhook_future_tolerance: Duration::from_secs(raw.webhook_future_tolerance_seconds),
...
impl WaffoConfiguration {
    ...
    pub const fn webhook_past_tolerance(&self) -> Duration {
        self.webhook_past_tolerance
    }

    pub const fn webhook_future_tolerance(&self) -> Duration {
        self.webhook_future_tolerance
    }
    ...
    pub const fn webhook_verification_configured(&self) -> bool {
        self.webhook_public_key.is_some() && self.store_id.is_some()
    }

    pub const fn webhook_registration_configured(&self) -> bool {
        self.store_id.is_some() && self.webhook_url.is_some() && !self.webhook_events.is_empty()
    }
...
const fn default_webhook_past_tolerance_seconds() -> u64 {
    2_700
}

const fn default_webhook_future_tolerance_seconds() -> u64 {
    60
}
...
"#;
    ...
    #[test]
    fn custom_api_origin_is_an_explicit_test_only_operator_seam() {
        let disabled = RawWaffoConfiguration {
            api_base_url: "https://127.0.0.1:9443".into(),
            merchant_id: "MER_0123456789ABCDEFGHIJKL".into(),
            private_key: SecretReferenceInput::Text("env:WAFFO_PRIVATE_KEY".into()),
            ..RawWaffoConfiguration::default()
        };
        assert!(matches!(
            WaffoConfiguration::try_from(disabled),
            Err(WaffoError::InvalidConfiguration(
                "custom api_base_url requires allow_custom_api_base_url = true"
            ))
        ));

        let enabled_for_test = RawWaffoConfiguration {
            api_base_url: "https://127.0.0.1:9443".into(),
        ...
        assert!(WaffoConfiguration::try_from(enabled_for_test).is_ok());

        let production = RawWaffoConfiguration {
            environment: WaffoEnvironment::Production,
            api_base_url: "https://127.0.0.1:9443".into(),
            allow_custom_api_base_url: true,
            merchant_id: "MER_0123456789ABCDEFGHIJKL".into(),
        ...
        assert!(matches!(
            WaffoConfiguration::try_from(production),
            Err(WaffoError::InvalidConfiguration(
                "production Waffo credentials may only target the official API origin"
            ))
        ));
    ...
    fn partial_webhook_configuration_fails_closed() {
        ...
        assert!(matches!(
            ...
            Err(WaffoError::MissingWebhookRegistrationConfiguration)
        ));
    }

    #[test]
    fn verification_only_configuration_does_not_require_registration_fields() {
        let raw = RawWaffoConfiguration {
            merchant_id: "MER_0123456789ABCDEFGHIJKL".into(),
            private_key: SecretReferenceInput::Text("env:WAFFO_PRIVATE_KEY".into()),
            store_id: Some("STO_0123456789ABCDEFGHIJKL".into()),
            webhook_public_key: Some(SecretReferenceInput::Text(
                "env:WAFFO_WEBHOOK_PUBLIC_KEY".into(),
            )),
            ..RawWaffoConfiguration::default()
        };

        let configuration = WaffoConfiguration::try_from(raw).unwrap();
        assert!(configuration.webhook_verification_configured());
        assert!(!configuration.webhook_registration_configured());
    }
}
