...
owned_paths:
  ...
  - tasks/M14/M14-T10-delivery-evidence-loop.md
checks:
...
- the authoritative local quality/release gates run the new evidence validator,
  targeted tests, complete workspace qualification and source-manifest
  verification, while the optional manual clean-Linux compatibility workflow
  remains bounded to `scripts/ci/hosted-essential.sh`;
...
toolchain. The authoritative `scripts/ci/local-release.sh` matrix also passes
from a clean JJ snapshot: workspace quality, 33 crate publication dry-runs,
unpacked-package consumers, CLI installation, deployment-plan rendering,
Lambda and worker builds, owned PostgreSQL/Rustack qualification, Rustack SDK
conformance and Orders E2E. Cargo explicitly aborted every upload because each
package operation was a dry-run. A deep diff security scan found and regression
covered source-authority TOCTOU, active-tree attachment, checkout-script
execution and malformed-ledger availability risks; the final focused code and
security reviews have no unresolved blocking finding. The final parallel CLI
matrix also exposed that a duplicated or inherited local-service lock
descriptor could outlive its Rust guard; explicit unlock-on-drop and a cloned
descriptor regression now bind the intended operation lifetime.

The committed performance candidate remains `NOT RUN`, the current provider
profile records no provider contact, and historical 0.4 evidence is `stale`;
therefore no production SLO or live-provider acceptance is claimed. The sealed
operational-validation receipt contains the two expected
`NOT RUN`/missing-provider warnings without embedding its own digest into the
source authority it identifies.
The first clean-history hosted release run (`31259616771`) passed the full
quality reproduction and then exposed a stale nested AppSync proof lockfile
before candidate qualification. Cargo refreshed only the three Minco path
package versions from `1.1.0` to `1.2.0`; the exact standalone proof then passed
locally. Before ADR-0038 replaced the full hosted profile, exact-head run
`31343110517` passed its full release matrix. Current final heads instead require
the authoritative local `scripts/ci/local-release.sh` matrix; the retained
manual clean-Linux workflow is a distinct bounded compatibility check, not a
substitute release gate or source authority.

The exact-tree hosted performance baseline and current live-provider proof
remain unavailable, so this task stays `active`. No AWS resource, deployment,
promotion, tag, release or publication was created by M14-T10.
