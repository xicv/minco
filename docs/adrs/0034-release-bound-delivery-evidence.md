# ADR-0034: Release-bound delivery evidence
...
receipt and rechecks each bound byte; it does not reclassify shallow TOML/JSON
fields on its own. It invokes the source-bound canonical validator through the
repository-pinned `uv run --locked` command and exact-byte compares the result;
missing `uv`, validator failure or a stale/forged receipt fails planning.
Generated baseline, operational-validation, handover,
feedback and sealed provider receipts are excluded narrowly and separately
bound to avoid self-reference. Policies, validators, ledgers, ADRs and research
stay inside source authority.
...
The candidate record is a strict `PASS`/`NOT RUN` union. M14-T07 records
