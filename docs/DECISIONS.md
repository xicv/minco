...
| [ADR-0067](adrs/0067-curated-views-macros-collision.md) | Five curated server-defined agent views (no query DSL), a project-scoped saved-reply library that is revision-aware (If-Match) and duplicate-title-safe, and advisory recent-viewer collision indication on agent detail. | Accepted |
| [ADR-0068](adrs/0068-assignment-modes-and-sla-snapshots.md) | Assignment decisions carry an explicit mode — manual, round-robin via a durable per-project cursor, least-workload with deterministic tie-break — picked from a configured bounded pool; SLA deadlines snapshot at creation and surface to agents only. | Accepted |
| [ADR-0069](adrs/0069-bounded-search-knowledge-csat.md) | Search is a bounded escaped-substring match over subject, reference and description (never message bodies); knowledge links are a bounded revision-gated replacement decision; CSAT is the requester's one-shot immutable rating on resolved tickets. | Accepted |
| [ADR-0071](adrs/0071-clarification-loop-with-checkpoints.md) | Clarification is a durable loop: agent-drafted bounded questions with opaque resume checkpoints, a human send decision, exactly-once own-ticket requester answers — checkpoints never cross to requesters, resume stays explicit. | Accepted |
| [ADR-0070](adrs/0070-private-development-automation.md) | Development automation is opt-in (off by default), runs as durable commands through Jobs, produces proposals that are never authority, enforces a fixed code-level exclusion list (merge/release/publish/deploy/production/secrets/dispatch), and keeps every automation state agent-only with exactly-once human decisions. | Accepted |
| [ADR-0072](adrs/0072-standalone-desk-example.md) | The standalone Minco Desk example composes every service on one SQLite database behind one native process with an explicit composition root; clean install, idempotent migration, health and an end-to-end lifecycle are proven in-process. | Accepted |
| [ADR-0073](adrs/0073-desk-data-lifecycle-proofs.md) | Desk data-lifecycle proofs on the real composition: backup via online `VACUUM INTO` with restore-serve, bounded FK-cascading retention erasure as an explicit operator operation, and durable job recovery across a simulated process death. | Accepted |
