...
together with durable auditing as the complete 34-package `v1.6.0`
...
- **published beta** is opt-in and usable with pinned exact 1.x packages. Its
...
| published beta plugin | `payments-waffo` | opt-in provider-specific integration in the 1.6.0 family; offline qualified with live-provider evidence still absent |
...
| Waffo hosted checkout | `plugin-payments-waffo` | signed typed actions, bounded checkout, raw-body webhook verification, no-network fakes and exact 1.6.0 publication | separately authorised live Waffo sandbox evidence |
