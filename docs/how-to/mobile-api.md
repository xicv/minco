...
tokens and API Gateway's JWT authorizer can reject invalid signatures, issuers,
audiences, and time claims before invoking Lambda. API Gateway can also enforce
explicitly configured route scopes, but Minco's current Plan records only
whether a route is public or authenticated. `minco-aws-lambda` maps the verified
claims into Minco's provider-neutral `Principal`; application use cases still
own permission checks, business scope, and authorization.
